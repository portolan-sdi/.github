...
def strip_labels(repo: str, number: int, token: str, allowed: set[str], norms_url: str) -> int:
...
def default_milestone(repo: str, number: int, token: str, action: str) -> None:
    ...
    request("PATCH", f"/repos/{repo}/issues/{number}", token, {"milestone": backlog["number"]})
